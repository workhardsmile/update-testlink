  ...
  def perform(script_name, testlink_ids)
    if testlink_ids.count > 0
      ...
      begin
        ...
        WHERE n.parent_id in (#{testlink_ids.join(',')}) AND v.field_id = 2 AND n.node_type_id = 4"
        ...
        if insert_testlink_ids.count > 0
          ...
          SELECT 2, '#{script_name}', n.id FROM nodes_hierarchy n WHERE n.node_type_id = 4 AND n.parent_id in (#{insert_testlink_ids.join(',')})"
        ...
        if update_testlink_ids.count > 0
          ...
          WHERE v.field_id = 2 AND v.node_id in
